...
#define xdigit(c) ((*(c)>='a')?*(c)-'a'+10:(*(c)>='A')?*(c)-'A'+10:*(c)-'0') 

static int luacrypto_hex2bin(lua_State *L)
{
    size_t i, len, blen = 0;
    const char *input = (char *)luaL_checklstring(L, 1, &len);
	blen = len / 2;
	unsigned char *bin = (unsigned char*)calloc(sizeof(char), blen + 1);	
	for (i = 0; i < blen; i++)
    {
		bin[i] = xdigit(input) * 16 + xdigit(input + 1);
		input += 2;
    }
	bin[blen] = '\0';
	lua_pushlstring(L, bin, blen);
    free(bin);
    return 1;
}
...
//added by jongnam (2014-07-18)
/*************** aes128 API ***************/

static char *pad_pkcs5(const char *data, size_t data_len, size_t *padded_len)
{
	char *padded_data;
	unsigned padding_len = AES_BLOCK_SIZE - (data_len % AES_BLOCK_SIZE);
	...
	padded_data = (char*)malloc(data_len + padding_len);
...
static int aes128_fencrypt(lua_State *L)
{
	size_t key_len = 0;
	size_t len = 0;
	const char *key = luaL_checklstring(L, 1, &key_len);
	const char *plain = (unsigned char *)luaL_checklstring(L, 2, &len);
	unsigned int i, n;
	...
	//padding 
	size_t padded_len;
	char *padded_data = pad_pkcs5(plain, len, &padded_len);

	//malloc encrpyt
	unsigned char *encrypt;
	encrypt = (unsigned char *)malloc(padded_len);

	unsigned char *pIn = (unsigned char *)padded_data;
	unsigned char *pOut = encrypt;

	n = 0;
	while (n < padded_len)
	{
		AES_ecb_encrypt(pIn + n, pOut + n, &stKey, AES_ENCRYPT);
		n += AES_BLOCK_SIZE;
	}

	lua_pushlstring(L, (char *)encrypt, (size_t)padded_len);
...
static int aes128_fdecrypt(lua_State *L)
{
	size_t key_len = 0;
	size_t len = 0;
	const char *key = luaL_checklstring(L, 1, &key_len);
	const char *encrypt = (unsigned char *)luaL_checklstring(L, 2, &len);
	int i, n;
	...
	//convert hex encrpyt to byte
	unsigned char *plain;

	plain = (unsigned char *)malloc(len);

	const unsigned char *pIn = (const unsigned char *)encrypt;
	unsigned char *pOut = plain;

	n = 0;
	while (n < len)
	{
		AES_ecb_encrypt(pIn + n, pOut + n, &stKey, AES_DECRYPT);
		n += AES_BLOCK_SIZE;
	}

	char padding = plain[len - 1];
	int padded_len = 0;
	if (padding > 0 && (int)padding <= AES_BLOCK_SIZE) {
		int c = 0;
		for (i = len - 1; i >= 0 && c < AES_BLOCK_SIZE; i--, c++) {
			if (plain[i] != padding) {
				break;
			}
			padded_len++;
	...
	if (padded_len == (int)padding) {
		plain[len - padding] = '\0';
	}

	lua_pushlstring(L, (char *)plain, (size_t)(len - padding));
...
static void create_metatables (lua_State *L)
{
	...
	struct luaL_Reg aes_functions[] =
	{
		{ "encrypt", aes128_fencrypt },
		{ "decrypt", aes128_fdecrypt },
		{ NULL, NULL }
	};
...
#define CALLTABLE(n) create_call_table(L, #n, n##_fnew, n##_f##n)
	...
	//added by jongnam (2014-07-18)
	luacrypto_register_submodule(L, "crypto.aes128", aes_functions);
