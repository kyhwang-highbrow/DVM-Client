...
int lua_cocos2dx_spine_SkeletonRenderer_setBlendFunc(lua_State* tolua_S)
{
    ...
    if (argc == 1) 
    {
        ...
        ok &= luaval_to_blendfunc(tolua_S, 2, &arg0);
        if(!ok)
        {
...
int lua_cocos2dx_spine_SkeletonRenderer_useBonePosition(lua_State* tolua_S)
{
    ...
    if (!cobj)
    {
        tolua_error(tolua_S, "invalid 'cobj' in function 'lua_cocos2dx_spine_SkeletonRenderer_useBonePosition'", nullptr);
        return 0;
    }
#endif
    ...
    if (argc == 1)
    {
        ...
        if (!ok)
        {
            tolua_error(tolua_S, "invalid arguments in function 'lua_cocos2dx_spine_SkeletonRenderer_useBonePosition'", nullptr);
            return 0;
        }
        
        cobj->useBonePosition(arg0);
        lua_settop(tolua_S, 1);
        return 1;
    }
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "sp.SkeletonRenderer:useBonePosition", argc, 1);
    return 0;

#if COCOS2D_DEBUG >= 1
tolua_lerror:
    tolua_error(tolua_S, "#ferror in function 'lua_cocos2dx_spine_SkeletonRenderer_useBonePosition'.", &tolua_err);
#endif

    return 0;
}

int lua_cocos2dx_spine_SkeletonRenderer_getBonePosition(lua_State* tolua_S)
{
    int argc = 0;
    spine::SkeletonRenderer* cobj = nullptr;
    bool ok = true;
    ...
    if (!cobj)
    {
        tolua_error(tolua_S, "invalid 'cobj' in function 'lua_cocos2dx_spine_SkeletonRenderer_getBonePosition'", nullptr);
        return 0;
    }
#endif
    argc = lua_gettop(tolua_S) - 1;
