...
static int lua_cocos2dx_experimental_WebView_setOnShouldStartLoading(lua_State *L)
{
    ...
    if (argc == 1)
    {
        ...
        std::function<bool(experimental::ui::WebView *sender, const std::string &url)> callback = [handler](experimental::ui::WebView *sender, const std::string &url)->bool {
...
static int lua_cocos2dx_experimental_WebView_setOnDidFinishLoading(lua_State *L)
{
    ...
    if (argc == 1)
    {
        ...
        LUA_FUNCTION handler = toluafix_ref_function(L, 2, 0);
        std::function<void(experimental::ui::WebView *sender, const std::string &url)> callback = [handler](experimental::ui::WebView *sender, const std::string &url) {
			lua_State *L = LuaEngine::getInstance()->getLuaStack()->getLuaState();
			toluafix_pushusertype_ccobject(L, sender->_ID, &(sender->_luaID), (void*)sender, "ccexp.WebView");
			tolua_pushcppstring(L, url);
