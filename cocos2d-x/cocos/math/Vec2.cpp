...
{
    CCASSERT(!(min.x > max.x || min.y > max.y ), "Vec2::clamp");
...
{
    CCASSERT(dst, "Vec2::clamp");
    CCASSERT(!(min.x > max.x || min.y > max.y ), "Vec2::clamp1");
