...
def start_upload():
    ...
    upload(sheet_name, spreadsheet_key, all_data_list, header, locale_list, is_scenario=True)


def extract():
    global search_root, ignore_files, ignore_folders, all_data_list, date_str, count, sheet_name, spreadsheet_id
    
    date = datetime.datetime.now()
    date_str = date.strftime(r'%Y.%m.%d %H:%M:%S')
    
    # 1. 시나리오 폴더 안 파일들로부터 데이터를 추출합니다
    from_scenario = extract_from_scenario(search_root + r'\..\data\scenario', ignore_files, ignore_folders)
