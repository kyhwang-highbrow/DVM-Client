...
def removeStr(sheet, data_list, header, locale_list, is_scenario=False): # sheet과 datalist의 값 비교를 통해 중복을 제거한 data_list 반환합니다.
    ...
    if is_scenario: # 시나리오 텍스트의 경우
        is_new_sheet = '_backup' in sheet.title
        ...
        for row in sheet.get_all_values():
            ...
            for index, data in enumerate(data_list):
                if data[data_text_kr_index] == row[row_text_kr_index]: # 이미 겹치는 대사가 있는 경우 
                    if is_new_sheet: # 뉴 시트인 경우에는 제외합니다
                        remove_index.append(index)
                    else: # 백업 시트인 경우 내용을 채워줍니다.
                        for locale in locale_list:
                            if locale in row_text_locale_index:
                                data[data_text_locale_index[locale]] = row[row_text_locale_index[locale]]
                if data[data_speaker_kr_index] == row[row_speaker_kr_index] and row[row_speaker_kr_index] != '' and not is_new_sheet: # 이미 겹치는 화자 이름이 있는 경우
...
def upload(sheet_name, spreadsheet_key, data_list, header, locale_list, is_scenario=False):
    ...
    row_count = len(exist_datas) + 1
    work_sheet.resize(rows=row_count)
    if len(data_list) > 0:
        work_sheet.insert_rows(data_list, row_count, value_input_option='RAW')
