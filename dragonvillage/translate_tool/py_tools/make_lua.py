...
def merge_all_data(work_sheets):
    ...
    all_data_list = [[] for _ in range(len(lua_table_config['table_file_name_list']))]
    ...
    print("Lua table text count :", len(all_data_list[0]))
...
def make_all():
    ...
    for index, lua_table_name in enumerate(lua_table_config['table_file_name_list']):
        print('Start make lua table :', lua_table_name)
