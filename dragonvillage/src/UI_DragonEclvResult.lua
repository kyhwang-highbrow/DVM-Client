...
-------------------------------------
-- class UI_DragonEclvResult
-------------------------------------
UI_DragonEclvResult = class(PARENT, {
     })
...
function UI_DragonEclvResult:init(dragon_object)
    ...
    -- 백키 지정
    g_currScene:pushBackKeyListener(self, function() self:click_exitBtn() end, 'UI_DragonEclvResult')
...
-------------------------------------
function UI_DragonEclvResult:click_exitBtn()
    self:close()
end
...
-- function refresh
-------------------------------------
function UI_DragonEclvResult:refresh(dragon_object)
...
-- @brief 능력치 정보 갱신
-------------------------------------
function UI_DragonEclvResult:refresh_status(dragon_object)
    local vars = self.vars

    local doid = dragon_object['id']

    local status_calc = MakeOwnDragonStatusCalculator(doid, {['eclv'] = dragon_object['eclv'] - 1})
