...
-------------------------------------
GameWorldArenaNew = class(PARENT, {
        m_enemyTamer = '',
...
-- function init
-------------------------------------
function GameWorldArenaNew:init(game_mode, stage_id, world_node, game_node1, game_node2, game_node3, ui, develop_mode, friend_match)
...
-------------------------------------
function GameWorldArenaNew:createComponents()
...
-- function initGame
-------------------------------------
function GameWorldArenaNew:initGame(stage_name)
    -- 구성 요소들을 생성
...
-- function initBG
-------------------------------------
function GameWorldArenaNew:initBG(waveMgr)
...
-- function initTamer
-------------------------------------
function GameWorldArenaNew:initTamer()
...
-- @brief 패시브 발동
-------------------------------------
function GameWorldArenaNew:passiveActivate_Right()
...
-------------------------------------
function GameWorldArenaNew:changeCameraOption(tParam, bKeepHomePos)
...
-------------------------------------
function GameWorldArenaNew:changeEnemyHomePosByCamera(offsetX, offsetY, move_time, no_tamer)
...
-- function onEvent
-------------------------------------
function GameWorldArenaNew:onEvent(event_name, t_event, ...)
...
-------------------------------------
function GameWorldArenaNew:prepareAuto()
...
-- @brief 전투영역 설정
-------------------------------------
function GameWorldArenaNew:setBattleZone(formation, immediately, is_right)
...
-------------------------------------
function GameWorldArenaNew:makeHeroDeck()
    -- 서버에 저장된 드래곤 덱 사용
    local user_info = g_gameScene:getStructUserInfo_Player()
...
-------------------------------------
function GameWorldArenaNew:makeEnemyDeck()
    local t_pvp_deck
    local l_deck
...
-- @brief 테이머 스킬 보기
-------------------------------------
function GameWorldArenaNew:print_tamer_skill()
...
-- return left, right
-------------------------------------
function GameWorldArenaNew:getRealtimeHpPercentage()
