...
-------------------------------------
-- class UI_ButtonFleaShop
-- @brief 관리되는 버튼 (버튼이 노출되는 여부에 따라 상위 메뉴에서 위치 변경)
-- @used_at 
-------------------------------------
UI_ButtonFleaShop = class(PARENT, {
        m_ncmId = 'number',
    })

-- 최초 1번은 노티를 보여주기 위함
UI_ButtonFleaShop.s_tOpenCnt = {}

-------------------------------------
-- function init
-------------------------------------
function UI_ButtonFleaShop:init(ncm_id)
    self.m_ncmId = ncm_id
    self:load('button_newcomer_shop_02.ui')
...
-- @brief
-------------------------------------
function UI_ButtonFleaShop:getOpenCnt()
    return UI_ButtonFleaShop.s_tOpenCnt[self.m_ncmId] or 0
end
...
-- @brief
-------------------------------------
function UI_ButtonFleaShop:incOpenCnt()
    if (UI_ButtonFleaShop.s_tOpenCnt[self.m_ncmId] == nil) then
        UI_ButtonFleaShop.s_tOpenCnt[self.m_ncmId] = 0
    end
    UI_ButtonFleaShop.s_tOpenCnt[self.m_ncmId] = (UI_ButtonFleaShop.s_tOpenCnt[self.m_ncmId] + 1)
end
...
-- @brief 버튼 클릭
-------------------------------------
function UI_ButtonFleaShop:click_btn()
    self:incOpenCnt()

    require('UI_FleaShop')
    local ui = UI_FleaShop(self.m_ncmId)
...
-------------------------------------
function UI_ButtonFleaShop:updateButtonStatus()
...
-- @brief 매 프레인 호출되는 함수
-------------------------------------
function UI_ButtonFleaShop:update(dt)
    ...
    do -- 3. 삭제될 건지 확인
        ...
        if (g_fleaShop:isActiveFleaShop(self.m_ncmId) == false) then
