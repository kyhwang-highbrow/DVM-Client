...
-------------------------------------
-- class UI_ArenaRankRewardItem
-------------------------------------
UI_ArenaRankRewardItem = class(PARENT,{
    })

-------------------------------------
-- function init
-------------------------------------
function UI_ArenaRankRewardItem:init(t_data)
...
-- @brief 레전드 티어 보상
-------------------------------------
function UI_ArenaRankRewardItem:init_legend(t_data, pure_tier, tier_grade)
...
-- @brief 마스터 티어 보상
-------------------------------------
function UI_ArenaRankRewardItem:init_master(t_data, pure_tier, tier_grade)
    local vars = self.vars
...
-- @brief 티어 보상
-------------------------------------
function UI_ArenaRankRewardItem:init_common(t_data, pure_tier, tier_grade)
    local vars = self.vars
    for i,v in ipairs(t_data['list']) do
        local cash = self:getCashCount(v)
...
-- @brief 입문자 티어 보상
-------------------------------------
function UI_ArenaRankRewardItem:init_beginner(t_data, pure_tier, tier_grade)
    local vars = self.vars
    local t_first = t_data['list'][1]
    local cash = self:getCashCount(t_first)
...
-------------------------------------
function UI_ArenaRankRewardItem:getCashCount(t_data)
