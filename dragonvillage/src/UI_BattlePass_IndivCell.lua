...
--------------------------------------------------------------------------
-- @classmod UI_BattlePass_IndivCell
-- @brief battle_pass_nurture_item.ui 와 관련된 item cell의 기능 동작 관리
--------------------------------------------------------------------------
UI_BattlePass_IndivCell = class(PARENT, {
    m_passId = 'number',
    m_passLevel = 'number',
    m_passExp = 'number',
...
UI_BattlePass_IndivCell.START_TYPE_IDX = 0
UI_BattlePass_IndivCell.END_TYPE_IDX = 2
--------------------------------------------------------------------------
-- @function init 
-- @param 데이터 테이블   {['id']=21; ['parent_key']=121701;}
-- @brief 모든 init 함수를 실행
--------------------------------------------------------------------------
function UI_BattlePass_IndivCell:init(data)
...
--------------------------------------------------------------------------
function UI_BattlePass_IndivCell:initUI()
end

--------------------------------------------------------------------------
-- @function initButton 
--------------------------------------------------------------------------
function UI_BattlePass_IndivCell:initButton()
    local vars = self.vars
    self.root:setSwallowTouch(false)
    vars['swallowTouchMenu']:setSwallowTouch(false)

    -- 보상 리스트
    for idx = UI_BattlePass_IndivCell.START_TYPE_IDX, UI_BattlePass_IndivCell.END_TYPE_IDX do
        -- 보상 버튼 표시
...
-- @function refresh 
--------------------------------------------------------------------------
function UI_BattlePass_IndivCell:refresh()
    local vars = self.vars
...
--------------------------------------------------------------------------
function UI_BattlePass_IndivCell:refreshReward(type_id)
    local vars = self.vars
