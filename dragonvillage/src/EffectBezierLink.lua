...
function EffectBezierLink:refreshEffect(tar_x, tar_y, pos_x, pos_y, dir)
	...
	for i, bezier_pos in ipairs(t_bezier_pos) do
		if (nil == self.m_lEffectNode[i]) then
			-- 없을 경우 생성
			effectNode = self:createWithParent(self.m_node, bezier_pos['x'], bezier_pos['y'], 0, self.m_resName, 'idle', false)
			effectNode.m_node:setAnchorPoint(cc.p(0.5, 0))
			effectNode.m_node:registerScriptLoopHandler(function()
				effectNode:changeAni('idle', true)
			end)
