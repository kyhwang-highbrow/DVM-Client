...
-------------------------------------
-- class UI_ClanBoardPopup_Write
-------------------------------------
UI_ClanBoardPopup_Write = class(PARENT,{
    })

local BOARD_MIN_LENGTH = 2
local BOARD_MAX_LENGTH = 200

-------------------------------------
-- function init
-------------------------------------
function UI_ClanBoardPopup_Write:init()
    local vars = self:load('clan_02_board_write.ui')
    UIManager:open(self, UIManager.POPUP)

    -- backkey 지정
    g_currScene:pushBackKeyListener(self, function() self:click_closeBtn() end, 'UI_ClanBoardPopup_Write')
...
-- function initUI
-------------------------------------
function UI_ClanBoardPopup_Write:initUI()
	...
	vars['editBox']:setMaxLength(BOARD_MAX_LENGTH)
...
-- function refresh
-------------------------------------
function UI_ClanBoardPopup_Write:refresh()
	local vars = self.vars
end
...
-------------------------------------
function UI_ClanBoardPopup_Write:validateEditText()
	local vars = self.vars
	...
	elseif (str_len > BOARD_MAX_LENGTH) then
		UIManager:toastNotificationGreen(Str('최대 글자수(200자)를 초과했어요!'))
		context = utf8_sub(context, BOARD_MAX_LENGTH)
...
-------------------------------------
function UI_ClanBoardPopup_Write:click_editBtn()
...
-------------------------------------
function UI_ClanBoardPopup_Write:click_closeBtn()
    self.m_closeCB = nil
	self:close()
end
...
function UI_ClanBoardPopup_Write:click_writeBtn()
	...
	g_clanData:request_writeBoard(finish_cb, fail_cb, context)
end

--@CHECK
UI:checkCompileError(UI_ClanBoardPopup_Write)
