...
------------------------------------
-- function isAvailGift
-- @brief 선물 보낼 수 있는지 여부
-- @param use_ui(boolean) 값이 true이면 불가이유를 토스트 메세지
-------------------------------------
function StructItem:isAvailGift(use_ui)
    -- 거래소에서 획득한 아이템의 경우
    if (self:isFromExchange() == true) then
        if (use_ui == true) then
            UIManager:toastNotificationRed(Str('거래소에서 구매한 아이템은 선물할 수 없습니다.'))
        end
        return false
    end

    -- 선물할 수 없는 단계의 레어도인 경우
    local max_rarity = TableBalanceConfig:getInstance():getPresentMaxRarity()
    local rarity = self:getGeneralFindDifficulty()
    if (max_rarity < rarity) then
        if (use_ui == true) then
            local str = TableGeneralRarity:getInstance():getRarityName(max_rarity + 1, false) -- params : find_difficulty ID, rich
            UIManager:toastNotificationRed(Str('{1} 희귀도 이상의 아이템은 선물할 수 없습니다.', str))
        end
        return false
    end

    -- 귀속 아이템의 경우
    if (self:isBinded() == true) then
        if (use_ui == true) then
            UIManager:toastNotificationRed(Str('귀속된 아이템은 선물할 수 없습니다.'))
        end
        return false
    end
