...
--------------------------------------------------------------------------
-- @classmod UI_BattlePass_Indiv
-- @brief 
--------------------------------------------------------------------------
UI_BattlePass_Indiv = class(PARENT, {
    m_passId = 'number',
...
UI_BattlePass_Indiv.START_TYPE_IDX = 0
UI_BattlePass_Indiv.END_TYPE_IDX = 2
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:getPassLevel()
...
-- @brief
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:initButton()
    local vars = self.vars
...
-------------------------------------
function UI_BattlePass_Indiv:refreshTableList()
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:refreshLevel()
    local vars =  self.vars
...
-- function update
-------------------------------------
function UI_BattlePass_Indiv:update()
    local vars =  self.vars
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:refreshButtons()
    local vars = self.vars
    ...
    for type_id = UI_BattlePass_Indiv.START_TYPE_IDX, UI_BattlePass_Indiv.END_TYPE_IDX do
        ...
        -- 가격 추가
        local buy_label_str = string.format('buyLabel%d', type_id)
        if vars[buy_label_str] ~= nil then
            local struct_product = struct_indiv_pass:getIndivPassProduct(type_id)
...
-- @brief
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:initTableView()
    local vars = self.vars
    ...
    local function create_func(ui, data)
        ...
        -- 보상 리스트
        for idx = UI_BattlePass_Indiv.START_TYPE_IDX, UI_BattlePass_Indiv.END_TYPE_IDX do
            -- 보상 버튼 표시
    ...
    table_view:setCellUIClass(UI_BattlePass_IndivCell, create_func)
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:click_infoBtn()
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:click_questBtn() 
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:click_buyBtn(type_id)
    local struct_indiv_pass = self.m_passData
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:click_rewardBtn(type_id, level)
    local struct_indiv_pass = self.m_passData
    if struct_indiv_pass:isIndivPassValidTime() == false then
        MakeSimplePopup(POPUP_TYPE.OK, Str('이벤트가 종료되었습니다.'))
        return
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:click_rewardLevelBtn(type_id, level, finish_cb)
    local struct_indiv_pass = self.m_passData
    if struct_indiv_pass:isIndivPassValidTime() == false then
        MakeSimplePopup(POPUP_TYPE.OK, Str('이벤트가 종료되었습니다.'))
        return
...
--------------------------------------------------------------------------
function UI_BattlePass_Indiv:click_itemInfoBtn(type_id)
    local struct_indiv_pass = self.m_passData
