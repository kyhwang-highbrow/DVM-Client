...
--------------------------------------------------------------------------
function ServerData_ClanRaidWaterPackage:isRewardReceived(product_id, dungeon_score)
    --if (not self.m_packageInfo[tostring(product_id)]) then return false end
    ...
    for i, v in pairs(package_table) do
        for key, data in pairs(v) do
            ...
            if self:isPackageActive(product_id) then
                ...
                and (g_clanRaidData:isClanDungeonWaterRewardAvailable(dungeon_score)) then
                    return true
                end
            else
                break
