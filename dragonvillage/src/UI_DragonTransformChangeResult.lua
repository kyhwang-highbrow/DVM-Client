...
-------------------------------------
-- class UI_DragonTransformChangeResult
-------------------------------------
UI_DragonTransformChangeResult = class(PARENT, {
        m_before_transform = 'number',
        m_dragon_object = '',
        m_dragon_animator = '',
     })
...
-- function init
-------------------------------------
function UI_DragonTransformChangeResult:init(before_transform, dragon_object)
    local vars = self:load('dragon_transform_result.ui')
    UIManager:open(self, UIManager.SCENE)
    self.m_before_transform = before_transform
...
-- @brief 능력치 정보 갱신
-------------------------------------
function UI_DragonTransformChangeResult:setResultText(dragon_object)
    local vars = self.vars

    local before = self.m_before_transform
    local after = dragon_object['transform']
...
-------------------------------------
function UI_DragonTransformChangeResult:showEvolutionEffect(dragon_object)
    local vars = self.vars

    self.m_dragon_object = dragon_object

    local did = dragon_object['did']
    local evolution = dragon_object['transform']
    ...
    -- 드래곤 하단 이펙트
    do 
        local visual = vars['changeVisual1']
    ...
    -- 드래곤 상단 이펙트
    do 
        local visual = vars['changeVisual2']
        visual:setVisible(true)
    ...
    do
        ...
        local dragon_animator = AnimatorHelper:makeDragonAnimator_usingDid(did, self.m_before_transform)
...
-- function showResult
-------------------------------------
function UI_DragonTransformChangeResult:showResult(immediately)
    local vars = self.vars
...
-------------------------------------
function UI_DragonTransformChangeResult:click_exitBtn()
    self:fadeOutClose()
end
...
-------------------------------------
function UI_DragonTransformChangeResult:click_skipBtn()
    local vars = self.vars 
...
-------------------------------------
function UI_DragonTransformChangeResult:fadeOutClose()
