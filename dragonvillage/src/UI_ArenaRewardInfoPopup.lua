...
function UI_ArenaRewardInfoPopup:onChangeTab(tab, first)
    ...
    -- 주간 랭킹 보상
    if (tab == UI_ArenaRewardInfoPopup.RANK) then
        self:init_tableViewRankReward()

    -- 매치 보상
    elseif (tab == UI_ArenaRewardInfoPopup.MATCH) then
        self:init_tableViewMatchReward()

    -- 클랜 주간 랭킹 보상
    elseif (tab == UI_ArenaRewardInfoPopup.CLAN) then
...
-- @brief 주간 랭킹 보상
-------------------------------------
function UI_ArenaRewardInfoPopup:init_tableViewRankReward()
...
-- @brief 매치 보상
-------------------------------------
function UI_ArenaRewardInfoPopup:init_tableViewMatchReward()
...
-------------------------------------
function UI_ArenaRewardInfoPopup:init_clanRewardTableView()
...
-------------------------------------
function UI_ArenaRewardInfoPopup:getItemList()
...
-- @brief 티어별 정렬
-------------------------------------
function UI_ArenaRewardInfoPopup:sort(target_list)
    local t_sort_idx = {}
    t_sort_idx['legend']   = 6
    t_sort_idx['master']   = 5
