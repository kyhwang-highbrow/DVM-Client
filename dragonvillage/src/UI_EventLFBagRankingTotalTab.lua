...
------------------------------------- 
-- class UI_EventLFBagRankingTotalTab
-------------------------------------
UI_EventLFBagRankingTotalTab = class(PARENT,{
...
-------------------------------------
function UI_EventLFBagRankingTotalTab:setParentAndInit(parent_node)
...
-- function initButton
-------------------------------------
function UI_EventLFBagRankingTotalTab:initButton()
...
-- function refresh
-------------------------------------
function UI_EventLFBagRankingTotalTab:refresh()
...
-------------------------------------
function UI_EventLFBagRankingTotalTab:refresh_playerUserInfo()
    local vars = self.vars
...
function UI_EventLFBagRankingTotalTab:makeRankRewardTableView()
    ...
    local l_item_list = g_eventLFBagData:getTotalRankRewardList()
...
-- @brief
-------------------------------------
function UI_EventLFBagRankingTotalTab:make_UIC_SortList()
    local vars = self.vars
...
function UI_EventLFBagRankingTotalTab.makeCellUIRankReward(t_reward_info)
    ...
    if (tonumber(t_reward_info['rank_min']) == 1) then
        ...
        local card_ui = MakeSimpleDragonCard(did, t_data)
            ...
            card_ui.root:setScale(100/150)
        vars['itemNode' .. #l_item_list + 1]:addChild(card_ui.root)
    end
...
--@CHECK
UI:checkCompileError(UI_EventLFBagRankingTotalTab)
