...
-------------------------------------
-- class UI_IranEmailLoginPopup
-- @brief 이란 빌드에서만 사용하는 이메일 로그인 팝업
-------------------------------------
UI_IranEmailLoginPopup = class(PARENT, {
    })

-------------------------------------
-- function init
-------------------------------------
function UI_IranEmailLoginPopup:init()
    local vars = self:load('iran_email_login_popup.ui')
    UIManager:open(self, UIManager.POPUP)
    
    -- backkey 없음
    g_currScene:pushBackKeyListener(self, function() self:click_closeBtn() end, 'UI_IranEmailLoginPopup')
...
-- function initUI
-------------------------------------
function UI_IranEmailLoginPopup:initUI()
...
-- function initButton
-------------------------------------
function UI_IranEmailLoginPopup:initButton()
    local vars = self.vars
...
-- function refresh
-------------------------------------
function UI_IranEmailLoginPopup:refresh()    
end

-------------------------------------
-- function click_closeBtn
-------------------------------------
function UI_IranEmailLoginPopup:click_closeBtn()
    self:close()
end
...
-- @brief 확인 버튼
-------------------------------------
function UI_IranEmailLoginPopup:click_loginBtn()
    -- email 확인
    -- pw 확인 (6자 이상인지, 같은지)
    ...
    local function response_status_cb(ret)
        ...
        if (status == -10) then
            MakeSimplePopup(POPUP_TYPE.OK, Str('입력 내용을 확인하세요.'))
            return true
        end

        if (status == -95) then
            MakeSimplePopup(POPUP_TYPE.OK, Str('등록되지 않은 이메일입니다.'))
            return true
        end

        if (status == -96) then
            MakeSimplePopup(POPUP_TYPE.OK, Str('잘못된 비밀번호입니다.'))
            return true
        end
    ...
    ui_network:setFullUrl(GetPlatformApiUrl() .. '/user/loginbyemail')
...
-------------------------------------
function UI_IranEmailLoginPopup:loginSuccess(info)
    local t_info = nil
    ...
    cclog('# UI_IranEmailLoginPopup:loginSuccess(info)')
...
--@CHECK
UI:checkCompileError(UI_IranEmailLoginPopup)
