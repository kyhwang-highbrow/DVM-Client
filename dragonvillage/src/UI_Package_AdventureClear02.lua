...
-------------------------------------
-- class UI_Package_AdventureClear02
-------------------------------------
UI_Package_AdventureClear02 = class(PARENT,{
    })

-------------------------------------
-- function init
-------------------------------------
function UI_Package_AdventureClear02:init(struct_product, is_popup)
    ...
    local vars = self:load('package_adventure_clear_02.ui')
    
    self.m_isPopup = is_popup or false
    if (is_popup) then
        UIManager:open(self, UIManager.POPUP)
        -- 백키 지정
        g_currScene:pushBackKeyListener(self, function() self:click_closeBtn() end, 'UI_Package_AdventureClear02')
    end
...
-- function refresh
-------------------------------------
function UI_Package_AdventureClear02:refresh()
    PARENT.refresh(self)
...
-- function initButton
-------------------------------------
function UI_Package_AdventureClear02:initButton()
    local vars = self.vars
...
-------------------------------------
function UI_Package_AdventureClear02:init_tableView()
    local vars = self.vars
    ...
    require('UI_Package_AdventureClearListItem02')
    table_view:setCellUIClass(UI_Package_AdventureClearListItem02, create_func)
    ...
    -- 보상 받기 가능한 idx로 이동
    local stage_id, idx = g_adventureClearPackageData02:getFocusRewardStage()
    if stage_id then
...
-------------------------------------
function UI_Package_AdventureClear02:click_buyBtn()
...
-------------------------------------
function UI_Package_AdventureClear02:request_serverInfo()
    local function cb_func()
        self:refresh()
    end

    g_adventureClearPackageData02:request_adventureClearInfo(cb_func)
end
