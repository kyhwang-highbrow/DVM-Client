...
-------------------------------------
-- class SceneGameEventArena
-------------------------------------
SceneGameEventArena = class(PARENT, {
...
-- function init
-------------------------------------
function SceneGameEventArena:init(game_key, stage_id, stage_name, develop_mode, friend_match)
    self.m_stageName = 'stage_colosseum'
    self.m_bDevelopMode = develop_mode
    self.m_bFriendMatch = friend_match or false

    self.m_sceneName = 'SceneGameEventArena'
...
-------------------------------------
function SceneGameEventArena:init_gameMode()
    self.m_stageID = ARENA_STAGE_ID
    self.m_gameMode = GAME_MODE_EVENT_ARENA
...
-- function onEnter
-------------------------------------
function SceneGameEventArena:onEnter()
    g_gameScene = self
...
-- function onExit
-------------------------------------
function SceneGameEventArena:onExit()
    PARENT.onExit(self)
...
-- function prepare
-------------------------------------
function SceneGameEventArena:prepare()
    -- 테이블 리로드(메모리 보안을 위함)
    ...
    self:addLoading(function()
        ...
        -- 레이어 생성
        self:init_layer()
        self.m_gameWorld = GameWorldEventArena(self.m_gameMode, self.m_stageID, self.m_worldLayer, self.m_gameNode1, self.m_gameNode2, self.m_gameNode3, self.m_inGameUI, self.m_bDevelopMode, self.m_bFriendMatch)
...
-------------------------------------
function SceneGameEventArena:prepareAfter()
...
-------------------------------------
function SceneGameEventArena:prepareDone()
...
-- @breif
-------------------------------------
function SceneGameEventArena:networkGameFinish(t_param, t_result_ref, next_func)
...
-------------------------------------
function SceneGameEventArena:networkGameFinish_response(ret, t_result_ref)
...
-------------------------------------
function SceneGameEventArena:networkGameFinish_response_user_info(ret, t_result_ref)
...
-------------------------------------
function SceneGameEventArena:networkGameFinish_response_modified_dragons(ret, t_result_ref)
...
-------------------------------------
function SceneGameEventArena:networkGameFinish_response_added_dragons(ret, t_result_ref)
...
-- @breif 드랍 보상 데이터 처리
-------------------------------------
function SceneGameEventArena:networkGameFinish_response_drop_reward(ret, t_result_ref)
