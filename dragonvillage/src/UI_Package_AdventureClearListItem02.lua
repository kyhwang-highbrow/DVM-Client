...
-------------------------------------
-- class UI_Package_AdventureClearListItem02
-------------------------------------
UI_Package_AdventureClearListItem02 = class(PARENT, {
        m_data = '',
    })
...
-- function init
-------------------------------------
function UI_Package_AdventureClearListItem02:init(data)
    self.m_data = data
    local vars = self:load('package_adventure_clear_item_02.ui')
...
-- function initUI
-------------------------------------
function UI_Package_AdventureClearListItem02:initUI()
...
-- function initButton
-------------------------------------
function UI_Package_AdventureClearListItem02:initButton()
    local vars = self.vars
...
-- function refresh
-------------------------------------
function UI_Package_AdventureClearListItem02:refresh()
    local vars = self.vars
    local data = self.m_data

    if g_adventureClearPackageData02:isActive() then
        local stage_id = data['stage']
        if g_adventureClearPackageData02:isReceived(stage_id) then
...
-------------------------------------
function UI_Package_AdventureClearListItem02:click_rewardBtn()
    local data = self.m_data
    local stage_id = data['stage']
    ...
    g_adventureClearPackageData02:request_adventureClearReward(stage_id, cb_func)
end
...
-- @brief 스테이지 바로가기 버튼
-------------------------------------
function UI_Package_AdventureClearListItem02:click_linkBtn()
    local data = self.m_data
    local stage_id = data['stage']
