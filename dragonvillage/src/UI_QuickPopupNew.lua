...
function UI_QuickPopupNew:init_adventureBtn()
    ...
    if (not g_contentLockData:isContentLock('world_raid')) then
        table.insert(l_content, 'world_raid')
    end
    ...
    for i, content in ipairs(l_content) do
        ...
        if (is_content_lock) then
            ...
            if content == 'story_dungeon' then
                local season_id, _ = g_eventDragonStoryDungeon:getStoryDungeonSeasonId()
