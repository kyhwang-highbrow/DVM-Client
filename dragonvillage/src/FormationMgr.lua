...
-- @brief
-------------------------------------
function FormationMgr:findNearTarget(x, y, range, count, l_remove)
    ...
    for i,v in pairs(self.m_frontCharList) do
        -- 제외 리스트 확인
        if (not l_remove[v.phys_idx]) then
            local dist = getDistance(x, y, v.pos.x + v.body.x, v.pos.y + v.body.y) - v.body.size
            -- 거리 확인
            if (dist <= range) then
                table.insert(l_char, {dist, v})
            end
        end
    end
    ...
    -- 정렬 (1번 인덱스는 distance)
    table.sort(l_char, function(a, b)
            return a[1] < b[1]
        end)

    -- char만 담겨있는 리스트 생성
    local t_ret = {}
    for i,v in ipairs(l_char) do
        if (count == -1) or (not count) or (1 <= count) then
            table.insert(t_ret, v[2])
        end
    end
...
-------------------------------------
-- function findBezierTarget
-- @brief
-------------------------------------
function FormationMgr:findBezierTarget(tar_x, tar_y, pos_x, pos_y, course)
    -- 베지어 곡선의 좌표값을 가져온다.
    ...
    for _, bezier_pos in pairs(t_bezier_pos) do
        t_ret = self:findNearTarget(bezier_pos['x'], bezier_pos['y'], leaf_body_size, 1, EMPTY_TABLE)
		if (#t_ret > 0) then
...
function FormationMgrDelegate:getTargetList(x, y, team_type, formation_type, rule_type, t_data)
    ...
    elseif (formation_type == 'x') or (formation_type == '') then
        local t_org_list_1 = self.m_globalCharList
        self:addList(t_ret, TargetRule_getTargetList(rule_type, t_org_list_1, x, y, t_data))

    else
        error("미구현 Formation Type!! : " .. type)
    end
