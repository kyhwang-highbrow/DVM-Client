...
--- @brief 덱 만들고 버프 부여하기
-------------------------------------
function GameWorldDealkingEvent:makeHeroDeck()
    -- 부모 함수 호출
...
-------------------------------------
--- @function applyEventDealkingStageBonus
--- @brief 스테이지 보너스 등록
-------------------------------------
function GameWorldDealkingEvent:applyEventDealkingStageBonus(dragon)
    local attr = TableStageData():getValue(self.m_stageID, 'attr')
    local l_buff = TableDealkingBuff:getInstance():getDealkingStageBuffList(self.m_stageID, attr)
