...
ServerData_FirstPurchaseEvent = class({
        ...
        --"first_purchase_event_info":{ // List
        --    "10001":{ // id
        --      "reward":"770814;1,700001;1000",  // 보상 아이템
        --      "end_date":1586697510000,         // 종료 날짜(계정생성일 + period)
        --      "is_reward":1,                    // 보상 수령 여부
        --      "t_name":"첫 충전 선물"
        --    },
        --    "10002":{ // id
        --      "reward":"770814;1,700001;1000",  // 보상 아이템
...
-- @brief
-------------------------------------
function ServerData_FirstPurchaseEvent:hasPurchasePointReward()
...
-- @brief
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePoint(version)
...
-- @brief
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchaseRewardStep(version)
...
-- @return sec
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePointEventRemainTime(version)
...
-- @return sec
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePointEventRemainTime_milliSecond(version)
    local purchase_point_info = self:getPurchasePointInfo(version)
...
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePointEventRemainTimeText(version)
...
-------------------------------------
function ServerData_FirstPurchaseEvent:getEventPopupTabList()
    local purchase_point_list = self.m_purchasePointInfo['purchase_point_list'] or {}
...
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePointInfo(version)
...
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePoint_rewardStepInfo(version, step)
    local purchase_point_info = self:getPurchasePointInfo(version)
    if (not purchase_point_info) then
        return nil
    end
...
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePoint_stepCount(version)
    local purchase_point_info = self:getPurchasePointInfo(version)
    if (not purchase_point_info) then
        return 0
    end
...
-- @breif 최종 보상 타입 반환
-------------------------------------
function ServerData_FirstPurchaseEvent:getLastRewardType(version)
...
-- @breif 최종 보상 설명 반환
-------------------------------------
function ServerData_FirstPurchaseEvent:getLastRewardDesc(version)
    local last_step = self:getPurchasePoint_stepCount(version)
    local t_last_reward = self:getPurchasePoint_rewardStepInfo(version, last_step)
...
-- @breif 최종 보상 받았는지 확인
-------------------------------------
function ServerData_FirstPurchaseEvent:isGetLastReward(version)
    local last_step = self:getPurchasePoint_stepCount(version)
...
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePointTime(version)
    local purchase_point_info = self:getPurchasePointInfo(version)
    if (not purchase_point_info) then
        return 0
    end
...
-- @breif
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePoint_lastStepPoint(version)
...
-- @breif
-------------------------------------
function ServerData_FirstPurchaseEvent:getPurchasePoint_step(version, step)
...
-------------------------------------
function ServerData_FirstPurchaseEvent:isActivePurchasePointEvent(version)
...
-- @breif
-------------------------------------
function ServerData_FirstPurchaseEvent:getStartTime(version)
    if (not version) then
        return nil
    end
...
-- @breif
-------------------------------------
function ServerData_FirstPurchaseEvent:getEndTime(version)
    if (not version) then
        return nil
    end
