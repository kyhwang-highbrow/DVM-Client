...
-------------------------------------
-- class UI_ButtonSpecialOfferProductNurture
-- @brief 관리되는 버튼 (버튼이 노출되는 여부에 따라 상위 메뉴에서 위치 변경)
-- @used_at 
-------------------------------------
UI_ButtonSpecialOfferProductNurture = class(PARENT, {
        m_bActive = 'boolean',
    })
...
-- function init
-------------------------------------
function UI_ButtonSpecialOfferProductNurture:init()
    self:load('button_special_offer_nurtune_product.ui')
...
-- function isActive
-------------------------------------
function UI_ButtonSpecialOfferProductNurture:isActive()
...
-------------------------------------
function UI_ButtonSpecialOfferProductNurture:update(dt)
...
-------------------------------------
function UI_ButtonSpecialOfferProductNurture:updateButtonStatus()
    ...
    -- 상점에서 특별 할인 상품을 받아온다.
    local struct_product, idx, bonus_num = g_shopData:getSpecialOfferProductNurture()
