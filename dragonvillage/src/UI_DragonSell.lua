...
function UI_DragonSell:click_dragonMaterial(t_dragon_data)
    ...
    if self.m_tSellTable[doid] then
        ...
        g_dragonsData:dragonMaterialWarning(oid, next_func, nil, '판매하시겠습니까?') -- param : oid, next_func, t_warning, warning_msg
    end
end
...
function UI_DragonSell:click_sellBtn()
	...
	local sell_cnt = table.count(self.m_tSellTable)
	if (sell_cnt <= 0) then
		UIManager:toastNotificationGreen(Str('1111111111111'))
		return
	end
