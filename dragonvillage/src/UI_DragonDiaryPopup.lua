...
-------------------------------------
-- class UI_DragonDiaryPopup
-------------------------------------
UI_DragonDiaryPopup = class(PARENT, {
        m_tableView = '',
		m_currRid = 'number',
...
function UI_DragonDiaryPopup:init(auto_close)
	...
	-- backkey 지정
	g_currScene:pushBackKeyListener(self, function() self:click_exitBtn() end, 'UI_DragonDiaryPopup')
...
-------------------------------------
function UI_DragonDiaryPopup:initParentVariable()
    -- ITopUserInfo_EventListener의 맴버 변수들 설정
    self.m_uiName = 'UI_DragonDiaryPopup'
    self.m_bUseExitBtn = true
    self.m_titleStr = Str('드래곤 성장일지')
end

-------------------------------------
-- function initUI
-------------------------------------
function UI_DragonDiaryPopup:initUI()
    local vars = self.vars

	-- 드래곤 표시
    local did = g_dragonDiaryData:getStartDragonDid()
...
-- function initButton
-------------------------------------
function UI_DragonDiaryPopup:initButton()
    local vars = self.vars
...
-- function refresh
-------------------------------------
function UI_DragonDiaryPopup:refresh(t_data, b_force)
	local vars = self.vars
    ...
    -- 넘버링
    local road_idx = g_dragonDiaryData:getDiaryIdx(t_data['rid'])
    local num_str = Str('{1}번째 임무입니다.', road_idx)
    vars['titleNumLabel']:setString(num_str)

    -- 목표
    local desc = Str(t_data['t_desc'])
    vars['descLabel']:setString(desc)

	-- 설명
	local desc = Str(t_data['t_speech'])
	vars['speechLabel']:setString(desc)
    ...
    -- 보상 상태에 따른 버튼 처리
    local reward_state = g_dragonDiaryData:getRewardState(t_data['rid'])
    vars['rewardBtn']:setVisible(reward_state == 'has_reward')
    vars['completeSprite']:setVisible(reward_state == 'already_done')
    vars['questLinkBtn']:setVisible((reward_state == 'not_yet') and (t_data['rid'] == g_dragonDiaryData:getFocusRid()))
end

-------------------------------------
-- function makeRoadTableView
-------------------------------------
function UI_DragonDiaryPopup:makeRoadTableView()
	local node = self.vars['listNode']

	local l_road_list = TableDragonDiary():getSortedList()
	...
	local function after_create_func(ui, t_data)
		...
		if (t_data['rid'] == g_dragonDiaryData:getDisplayRoad()) then
			self:selectCell(ui, t_data)
    ...
    do -- 테이블 뷰 생성
        ...
        do
            ...
            local rid = g_dragonDiaryData:getDisplayRoad()
            local curr_idx = g_dragonDiaryData:getDiaryIdx(rid)
...
-- @brief 보상 받기
-------------------------------------
function UI_DragonDiaryPopup:click_rewardBtn()
    local function cb_func(ret)
        -- 보상 획득
        ItemObtainResult(ret)
        
        -- 보상 받은 road의 셀 갱신 (보상 표시 제거)
        local curr_idx = g_dragonDiaryData:getDiaryIdx(self.m_currRid)
        local t_cell = self.m_tableView:getItem(curr_idx)
        self.refreshCell(t_cell['ui'], t_cell['data'])
        
        -- 다음 road로 UI 갱신
        local next_idx = g_dragonDiaryData:getDiaryIdx(g_dragonDiaryData:getFocusRid())
    ...
    g_dragonDiaryData:request_diaryReward(self.m_currRid, cb_func)
end
...
-- @brief 바로가기
-------------------------------------
function UI_DragonDiaryPopup:click_questLinkBtn()
    local t_road = TableDragonDiary():get(self.m_currRid)
...
-------------------------------------
function UI_DragonDiaryPopup:click_exitBtn()
    self:closeWithAction()
end
...
-- @brief 보상 아이콘 생성
-------------------------------------
function UI_DragonDiaryPopup.makeRewardCard(reward_node, t_reward, block_btn, allign)
...
-- @brief 테이블 셀 선택 콜백
-------------------------------------
function UI_DragonDiaryPopup:selectCell(ui, t_data, is_first)
	-- 해당 정보로 UI 갱신
	self:refresh(t_data)
...
-- @brief 테이블 셀 생성
-------------------------------------
function UI_DragonDiaryPopup.makeCellUI(t_data)
	local ui = class(UI, ITableViewCell:getCloneTable())()
	local vars = ui:load('dragon_diary_item.ui')
	local rid = t_data['rid']

	-- 퀘스트 순번
    local road_idx = g_dragonDiaryData:getDiaryIdx(rid)
	...
	-- 보상 아이콘
	vars['rewardNode']:removeAllChildren(true)
	UI_DragonDiaryPopup.makeRewardCard(vars['rewardNode'], t_data['t_reward'], 'be block', 'left')

    -- 보상 4개 이상일때 처리
    local reward_cnt = #t_data['t_reward']
    if (reward_cnt >= 4) then
        vars['numLabel']:setPositionX(-90)
        vars['rewardNode']:setPositionX(-45)
        vars['rewardNode']:setScale(0.28)
    end
    
    -- 진행중 및 보상 표시
    UI_DragonDiaryPopup.refreshCell(ui, t_data)
...
-- @brief 테이블 셀 갱신
-------------------------------------
function UI_DragonDiaryPopup.refreshCell(ui, t_data)
    if (not ui) then
        return
    end
	...
	-- 진행중
	vars['ingSprite']:setVisible(rid == g_dragonDiaryData:getFocusRid())

    -- 보상 상태에 따른 처리
    local reward_state = g_dragonDiaryData:getRewardState(rid)
