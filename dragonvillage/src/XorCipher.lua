...
local cipher = "11001010" -- must be eight digit binary number

--Returns the XOR of two binary numbers
function XorCipher:xor(a,b)
  local r = 0
  local f = math.floor
  for i = 0, 31 do
    local x = a / 2 + b / 2
...
function XorCipher:E(str)
	...
	for ch in str:gmatch(".") do
		local c = XorCipher:toBits(string.byte(ch))
	...
	for i = 1,#block do
		...
		local result = {}
		for i = 1,8,1 do
			table.insert(result,XorCipher:xor(ciphert[i],bitt[i]))
		end

		block[i] = string.char(XorCipher:toDec(table.concat(result)))
	end
