...
function UI_AttendanceContinuous:init()
    ...
    local function action_finish()
        local first_item = g_attendanceData.m_continuousAddedItems['items_list'][1]
        local message = Str('{1}일 차' .. ' ' ..  g_attendanceData.m_continuousTitleText, g_attendanceData.m_continuousTodayStep)
...
function UI_AttendanceContinuous:initUI()
    ...
    -- 텍스트 정보 출력
    vars['titleLabel']:setString(g_attendanceData.m_continuousTitleText)
    vars['dayLabel']:setString(Str('{1}일 차 ', g_attendanceData.m_continuousTodayStep))
    vars['helpLabel']:setString(Str(g_attendanceData.m_continuousHelpText))

    self:initTableView()

    do -- 오늘의 보상 표시
        vars['finalDayLabel']:setString(Str('{1}일 차 ', g_attendanceData.m_continuousTodayStep))
        local today_item = g_attendanceData.m_continuousStepList[g_attendanceData.m_continuousTodayStep]
...
-------------------------------------
function UI_AttendanceContinuous:initTableView()
    local node = self.vars['listNode']
    --node:removeAllChildren()

    local l_item_list = g_attendanceData.m_continuousStepList

    -- 생성 콜백
    local function create_func(ui, data)
        if (data['step'] <= g_attendanceData.m_continuousTodayStep) then
    ...
    table_view:setCellUIClass(UI_AttendanceContinuousListItem, create_func)
