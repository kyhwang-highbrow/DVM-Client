...
function CommonSummoning:makeInstance(owner, t_skill, t_data)
    ...
    local summonObj_id = t_skill['res_1'] -- 소환체 ID
    local t_summonObj = TableSummonObject():get(summonObj_id)

    cclog('nya1')
    if (not t_summonObj or not t_summonObj['type']) then return end
    cclog('nya2')
    local summon_data = summonObj_id .. ';' .. '1;Basic2;MC01;MC01;1'

    local dynamic_wave = DynamicWave(self.m_waveMgr, summon_data, 0)
    owner.m_world.m_waveMgr:summonCreature(dynamic_wave, t_summonObj['type'])


    --owner.m_world:createSummonObject(summon_id, 3)

	--creature:setMissile()

	--owner.m_world:addToUnitList(creature)
    --owner.m_world.m_worldNode:addChild(creature.m_rootNode)
end
