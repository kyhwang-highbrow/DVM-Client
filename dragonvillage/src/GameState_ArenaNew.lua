...
-------------------------------------
GameState_ArenaNew = class(PARENT, {
        m_bWin = 'boolean',
...
-- function init
-------------------------------------
function GameState_ArenaNew:init(world)
    -- 콜로세움은 제한시간 3분으로 고정
...
-------------------------------------
function GameState_ArenaNew:initEnrage()
...
-------------------------------------
function GameState_ArenaNew:initState()
    PARENT.initState(self)

    self:addState(GAME_STATE_START, GameState_ArenaNew.update_start)
    self:addState(GAME_STATE_WAVE_INTERMISSION, GameState_ArenaNew.update_wave_intermission)
    self:addState(GAME_STATE_SUCCESS, GameState_ArenaNew.update_success)
    self:addState(GAME_STATE_FAILURE, GameState_ArenaNew.update_failure)
    self:addState(GAME_STATE_RESULT, GameState_ArenaNew.update_result)
end

-------------------------------------
-- function update_start
-------------------------------------
function GameState_ArenaNew.update_start(self, dt)
...
-------------------------------------
function GameState_ArenaNew.update_wave_intermission(self, dt)
    local world = self.m_world
...
-------------------------------------
function GameState_ArenaNew.update_success(self, dt)
...
-------------------------------------
function GameState_ArenaNew.update_failure(self, dt)
    local world = self.m_world
...
-------------------------------------
function GameState_ArenaNew:disappearAllDragon()
...
-------------------------------------
function GameState_ArenaNew:appearEnemy()
    -- 드래곤들을 등장
    local world = self.m_world
...
-- function fight
-------------------------------------
function GameState_ArenaNew:fight()
    PARENT.fight(self)
...
-------------------------------------
function GameState_ArenaNew:checkToDieHighestRariry()
    return false
end
...
-------------------------------------
function GameState_ArenaNew:doDirectionForIntermission()
    local t_camera_info = {}
...
-------------------------------------
function GameState_ArenaNew:processTimeOut()
...
-- function pause
-------------------------------------
function GameState_ArenaNew:pause()
    PARENT.pause(self)
...
-- function resume
-------------------------------------
function GameState_ArenaNew:resume()
    PARENT.resume(self)
...
-------------------------------------
function GameState_ArenaNew:makeResultUI(is_win)
    local world = self.m_world
    ...
    elseif (stage_id == CLAN_WAR_STAGE_ID) then
        ...
        func_network_game_finish = function()
            g_arenaNewData:request_arenaFinish(is_win, self.m_fightTimer, func_ui_result)
        end
...
-- @brief 광폭화 적용
-------------------------------------
function GameState_ArenaNew:applyEnrage()
