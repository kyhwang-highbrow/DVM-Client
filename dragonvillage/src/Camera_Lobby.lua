...
-- function init
-------------------------------------
function Camera_Lobby:init(parent, cb_swipe_event)
    -- root node 생성
    ...
    --self.m_touchHandler = CameraTouchHandler_Lobby(self, self.m_rootNode)
    Camera_LobbySwipe(self.m_rootNode, cb_swipe_event)
end

-------------------------------------
-- function setContainerSize
-------------------------------------
function Camera_Lobby:setContainerSize(width, height)
...
-------------------------------------
function Camera_Lobby:addLayer(node, perspective_ratio, perspective_ratio_y)
    if (not node) then
        error()
    end
...
-------------------------------------
function Camera_Lobby:setPosition(x, y, b_adjust)
    --cclog('x : ' .. x, 'y : ' .. y)
    if b_adjust then
...
-- function adjustPos
-------------------------------------
function Camera_Lobby:adjustPos(x, y, zoom)
...
-- function setZoom
-------------------------------------
function Camera_Lobby:setZoom(zoom, b_adjust)
...
-- function adjustZoom
-------------------------------------
function Camera_Lobby:adjustZoom(zoom)
...
-- function initAdjust
-------------------------------------
function Camera_Lobby:initAdjust()
...
-------------------------------------
function Camera_Lobby:actionMoveAndZoom(duration, x, y, zoom, finish_cb)
    -- 실행중인 액션이 있으면 중지
    ...
    for _,data in ipairs(self.m_aLayerList) do
        ...
        local action = cc.MoveTo:create(duration, cc.p(pos_x * perspective_ratio, pos_y * perspective_ratio_y))
        node:runAction(cc.EaseInOut:create(action, 2))
    end
    ...
    -- 액션 종료 콜백 함수 등록
    local sequence = cc.Sequence:create(cc.DelayTime:create(duration), cc.CallFunc:create(function() self:stopAction() if finish_cb then finish_cb() end end))
...
-- function actionZoom
-------------------------------------
function Camera_Lobby:actionZoom(duration, zoom)
...
-- function actionMove
-------------------------------------
function Camera_Lobby:actionMove(duration, x, y)
...
-- function stopAction
-------------------------------------
function Camera_Lobby:stopAction()
