    ...
    local function coroutine_function(dt)
        ...
        do -- 축복 정보
            ...
            if g_lairData:isLairSeasonEnd() == true then
                co:work('# 축복 정보 초기화 중')                
                g_lairData:request_lairSeasonReset(co.NEXT, co.ESCAPE)
                if co:waitWork() then return end
            end
        end
        ...
        -- hard refresh
        co:work('# 핫타임 정보 받는 중')
        g_hotTimeData:request_hottime(co.NEXT, co.ESCAPE)
        if co:waitWork() then return end
        ...
        if (not TutorialManager.getInstance():checkFullPopupBlock()) then
            ...
            -- 우선순위: 긴급 팝업 > 신규 유저 D+1 푸시 > 게임 내 공지 > 누적 결제 이벤트 > 풀팝업
			-- =============================================
            ...
            do 
                ...
                do
                    cclog('# 긴급 중요 팝업 show')
                    g_fullPopupManager:show(FULL_POPUP_TYPE.EMERGENCY_PROMOTION, show_func)
                end
...
function UI_Lobby:update_highlight()
    ...
    local b_rune_gacha_ticket_highlight = g_runesData:isRuneTicketGachaAvailable()
    vars['runeForgeNotiSprite']:setVisible(g_highlightData:isHighlightRune() or 
                                            b_rune_gacha_highlight or 
                                            b_rune_gacha_ticket_highlight)
...
function UI_Lobby:update_worldRaid()
    ...
    local is_lock = g_contentLockData:isContentLock('world_raid')
    if is_lock == true or g_worldRaidData:isAvailableWorldRaid() == false then
        return false
    end
