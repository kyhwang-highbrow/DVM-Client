...
bool Java_com_perplelab_dvpublic_AppActivity_nativeIsLandScape(JNIEnv *env, jobject thisz)
{
...
bool Java_com_perplelab_dvpublic_AppActivity_nativeIsDebug(JNIEnv *env, jobject thisz)
{
    #ifdef NDEBUG
        return false;
    #else
...
bool Java_com_perplelab_dvpublic_AppActivity_nativeIsTestMode(JNIEnv *env, jobject thisz)
{
    if (!ConfigParser::getInstance()->isInit())
    {
        ConfigParser::getInstance()->readConfig();
...
int Java_com_perplelab_dvpublic_AppActivity_nativeLoginPlatform(JNIEnv *env, jobject thisz)
{
...
void Java_com_perplelab_dvpublic_AppActivity_nativeSDKEventResult(JNIEnv *env, jobject thisz, jstring id, jstring result, jstring info)
{
    jboolean isCopy1;
    jboolean isCopy2;
    jboolean isCopy3;
