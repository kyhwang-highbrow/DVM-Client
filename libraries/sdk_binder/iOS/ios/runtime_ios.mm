...
bool naverCafeIsShowGlink(int funcID) {
    return [[PerpleSDK sharedInstance] naverCafeIsShowGlink];
}

void naverCafeShowWidgetWhenUnloadSdk(int funcID, int isShowWidget) {
    [[PerpleSDK sharedInstance] naverCafeShowWidgetWhenUnloadSdk:(isShowWidget == 0 ? NO : YES)];
}

void naverCafeSetWidgetStartPosition(int funcID, const char* arg0, const char* arg1) {
    [[PerpleSDK sharedInstance] naverCafeSetWidgetStartPosition:[NSString stringWithUTF8String:arg0]
                                                           andY:[NSString stringWithUTF8String:arg1]];
}

void naverCafeStartWidget(int funcID) {
    [[PerpleSDK sharedInstance] naverCafeStartWidget];
}

void naverCafeStopWidget(int funcID) {
    [[PerpleSDK sharedInstance] naverCafeStopWidget];
}

void naverCafeStart(int funcID, int tapIndex) {
    [[PerpleSDK sharedInstance] naverCafeStart:tapIndex];
}

void naverCafeStop(int funcID) {
    [[PerpleSDK sharedInstance] naverCafeStop];
}

void naverCafeStartWrite(int funcID) {
    [[PerpleSDK sharedInstance] naverCafeStartWrite];
}

void naverCafeStartImageWrite(int funcID, const char* imageUrl) {
    [[PerpleSDK sharedInstance] naverCafeStartImageWrite:[NSString stringWithUTF8String:imageUrl]];
}

void naverCafeStartVideoWrite(int funcID, const char* videoUrl) {
    [[PerpleSDK sharedInstance] naverCafeStartVideoWrite:[NSString stringWithUTF8String:videoUrl]];
}

void naverCafeSyncGameUserId(int funcID, const char* gameUserId) {
    [[PerpleSDK sharedInstance] naverCafeSyncGameUserId:[NSString stringWithUTF8String:gameUserId]];
}

void naverCafeSetUseVideoRecord(int funcID, int isSetUseVideoRecord) {
    [[PerpleSDK sharedInstance] naverCafeSetUseVideoRecord:(isSetUseVideoRecord == 0 ? NO : YES)];
}

void naverCafeSetUseScreenshot(int funcID, int isSetUseScreenshot) {
    [[PerpleSDK sharedInstance] naverCafeSetUseScreenshot:(isSetUseScreenshot == 0 ? NO : YES)];
}

void naverCafeScreenshot(int funcID) {
    [[PerpleSDK sharedInstance] naverCafeScreenshot];
}

void naverCafeSetCallback(int funcID) {
    // start / stop / scheme / join / article / comment / vote / screenshot / record / error
    const int processId = [PerpleSDK getProcessId];
    [[PerpleSDK sharedInstance] naverCafeSetCallback:^(NSString *result, NSString *info) {
        if ([PerpleSDK isCurrentProcessId:processId]) {
            PerpleCore::OnSDKResult(funcID, [result UTF8String], [info UTF8String]);
        }
    }];
...
void naverCafeInitGlobalPlug(int funcID, const char* neoIdConsumerKey, int communityId, int channelID) {
    [[PerpleSDK sharedInstance] naverCafeInitGlobalPlug:[NSString stringWithUTF8String:neoIdConsumerKey] communityId:communityId channelID:channelID];
}

void naverCafeSetChannelCode(int funcID, const char* channelCde ) {
    [[PerpleSDK sharedInstance] naverCafeSetChannelCode:[NSString stringWithUTF8String:channelCde]];
}

const char* naverCafeGetChannelCode(int funcID) {
    NSString * channelCode = [[PerpleSDK sharedInstance] naverCafeGetChannelCode];
    return [channelCode cStringUsingEncoding:(NSUTF8StringEncoding)];
}

void naverCafeStartWithArticle(int funcID, int articleID) {
    [[PerpleSDK sharedInstance] naverCafeStartWithArticle:articleID];
}
#pragma mark - Google
