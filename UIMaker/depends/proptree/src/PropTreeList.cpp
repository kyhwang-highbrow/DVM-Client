...
void CPropTreeList::OnPaint()
{
	...
	// draw all items
	if (!m_pProp->GetRootItem()->GetChild())
	{
		memdc.SelectObject(m_pProp->GetBoldFont());
		memdc.SetTextColor(RGB(0, 0, 0));
		memdc.SetBkMode(TRANSPARENT);

		memdc.DrawText(_T("No Selected"), rc, DT_SINGLELINE | DT_VCENTER | DT_CENTER);
	}
	else
	{
...
void CPropTreeList::OnLButtonUp(UINT, CPoint point) 
{
	if (m_bColDrag)
	{
		...
		if(point.x < PROPTREEITEM_EXPANDCOLUMN + MIN_ITEM_WIDTH) point.x = PROPTREEITEM_EXPANDCOLUMN + MIN_ITEM_WIDTH;
		if(rc.Width() - point.x < MIN_ITEM_WIDTH) point.x = rc.Width() - MIN_ITEM_WIDTH;
