...
BEGIN_MESSAGE_MAP(CPropTreeItemColor, CEdit)
...
/*	CBrush br(m_cColor);
	...
	*/
	...
	pDC->FillSolidRect(&rccolor, m_cColor);
...
void CPropTreeItemColor::OnActivate(CPoint pt)
{
	m_cPrevColor = m_cColor;

	CRect rcColor(m_rc);
	rcColor.right = rcColor.left + rcColor.Height();
	if (rcColor.PtInRect(pt))
	{
		CRect r(m_rc);
		r.right = r.left + 150;
		r.bottom = r.top + 120;
...
void CPropTreeItemColor::OpenEdit()
{
	CPropTreeItem::OpenEdit();

	CRect rcedit(m_rc);
	rcedit.left += rcedit.Height();
	...
	SetWindowPos(m_pProp->GetCtrlParent(), rcedit.left, rcedit.top, rcedit.Width(), rcedit.Height(), SWP_NOZORDER | SWP_SHOWWINDOW);
	SetFocus();

	m_bOpenEdit = TRUE;
}

void CPropTreeItemColor::CloseEdit()
{
	if (!m_bOpenEdit) return;

	char szbuf[256];
	GetWindowTextA(GetSafeHwnd(), szbuf, sizeof(szbuf) - 1);

	int i = 0;
	int c[3] = { GetRValue(m_cColor), GetGValue(m_cColor), GetBValue(m_cColor) };
	const char* delimiters = ", ";
	char* context = NULL;
	for (char* token = strtok_s(szbuf, delimiters, &context); token && i < 3; token = strtok_s(NULL, delimiters, &context), ++i)
	{
		c[i] = atoi(token);
	}

	m_cColor = RGB(c[0], c[1], c[2]);
...
void CPropTreeItemColor::OnPaint() 
{
	...
	for (LONG i=0; i<40; i++)
	{
		...
		_crColors[i].rcSpot = CRect(pt.x, pt.y, pt.x + 13, pt.y + 13);
	}
...
BOOL CPropTreeItemColor::PreTranslateMessage(MSG* pMsg)
{
...
void CPropTreeItemColor::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar)
	{
	case VK_TAB:
