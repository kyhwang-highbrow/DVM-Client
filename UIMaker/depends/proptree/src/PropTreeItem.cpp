...
void CPropTreeItem::Activate(CPoint pt)
{
	m_bActivated = TRUE;
	m_bCommitOnce = FALSE;
...
void CPropTreeItem::OnActivate(CPoint pt)
{
	// no attributes, do nothing
}


void CPropTreeItem::OpenEdit()
{
	m_bActivated = TRUE;
	m_bCommitOnce = FALSE;
}

void CPropTreeItem::OpenPrevEdit()
{
	auto item = m_pProp->FocusPrev();
	while (item && (item->IsRootLevel() || item->IsReadOnly()))
	{
		item = m_pProp->FocusPrev();
	}
	if (!item)
	{
		item = m_pProp->FocusFirst();
		while (item && (item->IsRootLevel() || item->IsReadOnly()))
		{
			item = m_pProp->FocusNext();
		}
	}
	if (!item) return;
...
void CPropTreeItem::OpenNextEdit()
{
	auto item = m_pProp->FocusNext();
	while (item && (item->IsRootLevel() || item->IsReadOnly()))
	{
		item = m_pProp->FocusNext();
	}
