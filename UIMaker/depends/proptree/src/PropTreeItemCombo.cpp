...
void CPropTreeItemCombo::OnActivate(CPoint pt)
{
	if (m_eFormat == ValueFormat_BOOL)
	{
		LONG idx;
		if ((idx = GetCurSel()) == CB_ERR)
			m_lComboData = 0;
...
void CPropTreeItemCombo::OpenEdit()
{
...
BOOL CPropTreeItemCombo::CreateComboBoxBool()
{
	...
	DWORD dwStyle = WS_CHILD|WS_VSCROLL|CBS_DROPDOWNLIST;
	...
	m_idxFalse = AddString(_T("False"));
	SetItemData(m_idxFalse, FALSE);

	s.LoadString(IDS_TRUE);
	m_idxTrue = AddString(_T("True"));
	SetItemData(m_idxTrue, TRUE);

	m_eFormat = ValueFormat_BOOL;
...
CPropTreeItemCombo::ValueFormat CPropTreeItemCombo::GetFormat()
{
	return m_eFormat;
}

void CPropTreeItemCombo::SetFormat(ValueFormat v)
{
	m_eFormat = v;
}


BOOL CPropTreeItemCombo::PreTranslateMessage(MSG* pMsg)
{
...
void CPropTreeItemCombo::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar)
	{
	case VK_TAB:
