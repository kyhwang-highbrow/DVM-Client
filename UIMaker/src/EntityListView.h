...
class CEntityListView : public CWnd
{
	DECLARE_DYNAMIC(CEntityListView)

public:
	CEntityListView();
	virtual ~CEntityListView();
...
protected:
	...
	void UpdateResize();
	void UpdateScroll();
	int GetTotalHeight(const maker::Entity* entity);
	int GetScrollPos(const maker::Entity* entity, const maker::Entity* current, bool& found);
	maker::Entity* hitReleaseEntity(const POINT& pt, int& hit_part);
	maker::Entity* hitEntity(const POINT& pt, int& hit_part);
	maker::Entity* findHitEntity(CRect rect, const maker::Entity* entity, const POINT& pt, int& count, CRect& rchititem, int depth, bool select_only_node);
	...
	CMenu m_MenuPopup;
	maker::CMD m_popup_cmd;

	void OnCMD_CreateEntity(int type);
	void OnCMD_PickEntity(int index);
	...
	void DrawEntity(CDC* pDC, CRect rect, int& count, const maker::Entity* entity, int depth, bool invisible);
	void DrawDragEntity(CDC* pDC, CRect rect, const maker::Entity* entity);

	std::string GetEntityInfo(const maker::Entity* entity) const;
...
public:
	...
	afx_msg void OnContextMenu(CWnd* pWnd, CPoint point, bool callFromViewer, maker::CMD* cmd = nullptr);
	...
	afx_msg void OnCMD_GridOnOff();
	afx_msg void OnCMD_GridOpacity();
	afx_msg void OnCMD_ResetZoom();
	afx_msg void OnCMD_ResetScroll();
